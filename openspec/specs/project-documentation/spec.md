...
リポジトリのトップレベルドキュメント(README、補助ドキュメント、ライセンス)がOSSプロジェクトとして満たすべき内容・言語・構成を担う。`README.md`と`docs/`配下の補助ドキュメントの記述言語、README冒頭のASCIIワードマーク表示、OSSライセンスの同梱を提供する(lazycurl本体の実行時の振る舞いは対象外)。
