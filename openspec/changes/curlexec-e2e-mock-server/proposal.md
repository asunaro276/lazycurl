...
- `teatest`(`github.com/charmbracelet/x/exp/teatest`)を用いたTUIレベルのE2Eテストを`internal/tui`に追加する。`tui.App`を実際の`tea.Program`として動かし、キー入力でCollectionsからRequestを選択・送信し、モックサーバーコンテナへの実curl実行結果がResponseパネルに描画されることを検証する
