...
- [ ] 3.2 ローカルで`docker build`してイメージが起動し、`/status/200`等に応答することを確認する — 実装環境のサンドボックスがDocker Hubレジストリへの外向き通信を組織ポリシーで拒否している(`golang:1.24-alpine`のpullが403)ため未確認。`docker build -t lazycurl-mockserver testing/mockserver && docker run --rm -p 8080:8080 lazycurl-mockserver` でレジストリ到達可能な環境にて確認要

## 4. docker composeによる手動起動

- [x] 4.1 `docker-compose.yml`を追加し、`testing/mockserver/Dockerfile`をbuildして固定ホストポートで公開する設定を書く
- [ ] 4.2 `docker compose up`で起動したモックサーバーに対し、lazycurlのTUI(または`curl`直接)からリクエストが通ることを手動確認する — 3.2と同じ理由(レジストリ到達不可)で未確認
...
このタスクを実装したサンドボックス環境は、組織のネットワークポリシーによりDocker Hubレジストリ(`production.cloudfront.docker.com`等)への到達がブロックされている(CONNECT時に403)。そのため:
- `docker build`/`docker compose up`(3.2, 4.2)によるモックサーバーイメージの実起動確認
- `testcontainers-go`によるE2Eテスト(6.x, 7.x)の実際のテスト実行(`go test ./internal/curlexec/...`, `go test ./internal/tui/...`)

はいずれも未実施。ただし`testcontainers-go`がDockerデーモンに接続し、`testing/mockserver/Dockerfile`のビルドを開始するところまでは実機のDockerデーモンで確認済みで、失敗点は`golang:1.24-alpine`ベースイメージのpull(レジストリポリシー拒否)のみ。`testing/mockserver`モジュール自体の単体テスト(`go test ./testing/mockserver/...`, 2.7)はDocker不要のため実行・全件パス済み。レジストリに到達可能な環境(通常のCI/開発機)であれば上記は問題なく動作する見込み。
