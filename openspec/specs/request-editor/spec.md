...
lazycurlはMethod/URL/Query Params/Headers/Authをフォーム形式(キー・バリューのグリッド編集、行の追加・削除・有効/無効切り替え)で編集できなければならない(SHALL)。生の`.http`テキストを直接編集することを必須としてはならない。KVGridは非編集時、選択中の行だけでなく選択中のセル(KeyまたはValue)を視覚的に区別して表示しなければならない(SHALL)。
